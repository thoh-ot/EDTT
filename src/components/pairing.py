...
class Pairing:
    ...
    def __random(self, idx):
        ...
        success = status == 0;
        event = get_event(self.transport, idx, 100)[1];
        ...
        while success:
            ...
            if not self.initiator.peer is None:
                ...
                while success:
                    ...
                    showEvent(event, eventData, self.trace);
                    success = subEvent == MetaEvents.BT_HCI_EVT_LE_LTK_REQUEST;
                    if not success:
                        break;
                    handle = struct.unpack('<H', eventData[1:3])[0];
                    ...
                    event, subEvent, eventData = get_event(self.transport, self.initiator.peer, 100)[1:];
                    showEvent(event, eventData, self.trace);
                    success = (event == Events.BT_HCI_EVT_ENCRYPT_CHANGE) or (event == Events.BT_HCI_EVT_ENCRYPT_KEY_REFRESH_COMPLETE);
                    if not success:
                        break;
                    if event == Events.BT_HCI_EVT_ENCRYPT_CHANGE:
                        status, handle, enabled = struct.unpack('<BHB', eventData[:4]);
                        success = (status == 0) and (enabled == 1);
                    else:
                        status, handle = struct.unpack('<BH', eventData[:4]);
                        success = status == 0;
                    break;
